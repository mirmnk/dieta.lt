...
class tx_vvmaistoforas2_models_pValueSingleView extends tx_lib_object {
	
	function loadProductByUid($fields='*', $uid=0) {
		$table = 'tx_vvmaistoforas2_basicproduct';
		$groupBy = null;
		$orderBy = null;
		$cObj = $this->findCObject();
		$pUid = ($uid)?$uid:intval($this->controller->parameters->get('pid'));
		...
		$product = tx_div::makeInstance('tx_lib_object');
		if($result) {
			$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result);
			$product->setArray($row);
	...
	function loadUnitsTable() {
		...
		if(is_array($rows) && count($rows)) {
			$this->set('units', new tx_lib_object($this->controller, $rows));		
		}
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_maistoforas2/models/class.tx_vvmaistoforas2_models_pValueSingleView.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_maistoforas2/models/class.tx_vvmaistoforas2_models_pValueSingleView.php']);
} 
?>
