...
class ux_tx_chcforum_display extends tx_chcforum_display {
	 ...
	 * @return void
	 */
	function tx_chcforum_display (&$cObj) {
		...
		$this->user = t3lib_div::makeInstance("tx_chcforum_user", $this->cObj);
		...
		if ($this->cat_uid) $this->cat = t3lib_div::makeInstance("tx_chcforum_category", $this->cat_uid, $this->cObj);
		if ($this->conf_uid) $this->conference = t3lib_div::makeInstance("tx_chcforum_conference", $this->conf_uid, $this->cObj);
		if ($this->thread_uid) $this->thread = t3lib_div::makeInstance("tx_chcforum_thread", $this->thread_uid, $this->cObj);
		if ($this->post_uid) $this->post = t3lib_div::makeInstance("tx_chcforum_post", $this->post_uid, $this->cObj);
		...
		// variables.
		$this->form = t3lib_div::makeInstance("tx_chcforum_form", $this);
		...
		if ($this->conf['cwtCommunityIntegrated.'] = true) {
			...
			switch($this->action) {
				...
				case 'getviewprofile':
					...
					$this->view = 'profile';
					$this->author = intval(t3lib_div::_GP('uid'));
					break;
				...
				case 'getviewmessagessingle':
					...
					$submitPressed = intval(t3lib_div::_GP('tx_cwtcommunity_pi1'));
	...
	function validate_path_vars() {
		...
		if ($this->post_uid && $this->post->is_valid == true) {
			if (empty($this->post)) $this->post = t3lib_div::makeInstance("tx_chcforum_post", $this->post_uid, $this->cObj);
			if ($this->thread_uid != $this->post->thread_id) $this->thread_uid = intval($this->post->thread_id);
			if ($this->conf_uid != $this->post->conference_id) $this->conf_uid = intval($this->post->conference_id);
			if ($this->cat_uid != $this->post->category_id) $this->cat_uid = intval($this->post->category_id);
		} else {
			if ($this->thread_uid) {
				if (empty($this->thread)) $this->thread = t3lib_div::makeInstance("tx_chcforum_thread", $this->thread_uid, $this->cObj);
				if ($this->conf_uid != $this->thread->conference_id) $this->conf_uid = intval($this->thread->conference_id);
				if ($this->cat_uid != $this->thread->category_id) $this->cat_uid = intval($this->thread->category_id);
			} else {
				if ($this->conf_uid) {
					if (empty($this->conference)) $this->conference = t3lib_div::makeInstance("tx_chcforum_conference", $this->conf_uid, $this->cObj);
					if ($this->cat_uid != $this->conference->cat_id) $this->cat_uid = intval($this->conference->cat_id);
				}
			}
		}
	}
	...
	function editProfile() {
		...
		if ($this->submit && $this->profile['submit'] == 'submit') {
			...
			$this->author = t3lib_div::makeInstance("tx_chcforum_author", $this->author_uid, $this->cObj);
	 ...
	 */
	function profile() {
		$this->html_out .= $this->return_sub_tool_bar();
		if ($this->author) {
			if ($this->author == 'self') {
			...
			if ($this->user->uid == $this->author_obj->uid && $this->submit && $this->profile['submit'] == 'submit') {
				$this->html_out .= $this->user->process_profile_form($this->profile, $this->files, $this->max_user_img);
				$this->author_obj = t3lib_div::makeInstance("tx_chcforum_author", $this->author, $this->cObj); // reset the author
			}
			...
			$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $text, 'nav_path');
	...
	function cwt_message() {
		...
		switch ($this->action) {
			case 'getviewmessagesnew':
				...
				$recipient = t3lib_div::makeInstance("tx_chcforum_author", $this->recipient_uid,$this->cObj);
		...
		$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $text, 'error_no_border');
		$this->html_out .= $this->return_sub_tool_bar();
	...
	function user_list() {
		...
		if (($this->fconf['restrict_ulist'] != false && !$this->user->uid) or ($this->fconf['ulist_disable'] == true)) {
			...
			$type = 'error';
			$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $msg, $type);
			// Output the nav path
			$this->html_out .= $this->set_nav_path();
			// Output the error.
			$this->html_out .= $message->display();
		...
		if ($this->conf['cwtCommunityIntegrated'] == true) {
			...
			$page_links_msg = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $this->page_links, 'page_links_top');
			...
			if ($count > 0) {
				while ($this->page_start <= $this->page_end) {
					$author = t3lib_div::makeInstance("tx_chcforum_author", $uids[$this->page_start],$this->cObj);
			...
			// Output the page links
			$page_links_msg = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $this->page_links, 'page_links_btm');
			$this->html_out .= $page_links_msg->display();
	...
	function list_posts($post_ids) {
		...
		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'single_thread.tpl');
		$tmpl->prepare();
		$tmpl->assign($markers);
		$this->html_out = $tmpl->getOutputContent();
	}
	...
	function list_threads($threads,$message = false) {
		...
		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'conf_view.tpl');
		$tmpl->prepare();
		...
		if ($this->thread_cnt > 0) {
			...
			while ($this->page_start <= $this->page_end) {
				$thread = t3lib_div::makeInstance("tx_chcforum_thread", $threads[$this->page_start], $this->cObj);
			...
			$no_new_msg = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('no_new_posts'), 'message');
			$tmpl->assignGlobal('message',$no_new_msg->display());
		}

		// Create the top and bottom page links
		$page_links_msg_t = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $this->page_links, 'page_links_top');
		$page_links_msg_b = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $this->page_links, 'page_links_btm');
	...
	function search() {
		$search_obj = t3lib_div::makeInstance("tx_chcforum_search", $this->search,$this->cObj);
		$results = $search_obj->display();
		// if it's an array, we have results
		if (is_array($results)) {
			// set search query
			$this->s_query = $results['query'];
			$no_new_msg = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('search_results_msg'), 'message');
	...
	function single_conf () {
		...
		if ($this->cat_uid && $this->conf_uid && $this->user->can_read_conf($this->conf_uid) == true) {
			...
			if ($threads) {
				...
				// Output the bottom page links

			} else {
				$message_text = tx_chcforum_shared::lang('error_no_threads');
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $message_text, 'error');
	...
	function return_sub_tool_bar() {
		...
		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'sub_tool_bar.tpl');
		$tmpl->prepare();
		...
		if ($this->conf['cwtCommunityIntegrated'] == true && $this->user->uid) {
	...
	function single_thread () {
		if ($this->cat_uid && $this->conf_uid && $this->user->can_read_conf($this->conf_uid) == true) {
			...
			$text = tx_chcforum_shared::makeLink($link_params, $link_text);
			$link_to_conf = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $text, 'link');
	...
	function edit_post () {
		if ($this->conf_uid && $this->user->can_edit_post($this->post_uid) == true) {
			...
			$text = tx_chcforum_shared::makeLink($link_params, $link_text);
			$link_to_thread = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $text, 'link');
			// Set up the form -- process any incoming variables
			$this->form->where = 'edit_post';
			$this->html_out .= $this->set_nav_path();
			$post = t3lib_div::makeInstance("tx_chcforum_post", $this->post_uid, $this->cObj);
	...
	function single_post () {
		if ($this->cat_uid && $this->conf_uid && $this->user->can_read_conf($this->conf_uid) == true) {
			...
			$this->form->subject = preg_replace('/(re: ){2,}/i', 're: ', $this->form->subject);			
			...
			// output this bad boy
			$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'single_post_view.tpl');
			$tmpl->prepare();
			$tmpl->assign($markers);
			$this->html_out = $tmpl->getOutputContent();
	...
	function all_cats () {
		...
		if (!empty($cat_ids)) {
			$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'cat_view.tpl');
			$tmpl->prepare();
			...
			foreach ($cat_ids as $a_cat_id) {
				$current_cat = t3lib_div::makeInstance("tx_chcforum_category", $a_cat_id, $this->cObj);
				$cat_header_row = $current_cat->cat_header_row();
				$cat_header_row['cat_title'] = tx_chcforum_shared::makeLink(
															array('view' => 'single_cat', 'cat_uid' => $a_cat_id), 
															$cat_header_row['cat_title'],
															'style="font-size: 110%;"'
												);
				...
				if ($conf_ids) {
					...
					foreach ($conf_ids as $a_conf_id) {
						if ($this->user->can_read_conf($a_conf_id) == true) {
							$current_conf = t3lib_div::makeInstance("tx_chcforum_conference", $a_conf_id, $this->cObj);
...
#			$tmpl->assignGlobal('nav_path',$this->set_nav_path());
	...
	function return_user_count() {
		if(!$this->user_count) {
			if (is_numeric($this->fconf['feusers_pid'])) $pid = $this->fconf['feusers_pid'];
			$query = 'SELECT COUNT(*) FROM fe_users WHERE deleted=0 AND disable = 0 AND pid=\''.$pid.'\'';
			...
			$real_count = $count[0];
			$this->user_count = $real_count;
		} else {
			$real_count = $this->user_count;
		}
		return $real_count;
	}
	...
	function return_post_count() {
		if(!$this->post_count) {
			...
			$real_count = $count[0];
			$GLOBALS['TYPO3_DB']->sql_free_result($results);
			$this->post_count = $real_count;
		} else {
			$real_count = $this->post_count;
		}
		return $real_count;
	}
	...
	function return_title_hdr() {
		if ($this->fconf['forum_title']) {
			$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'header.tpl');
			$tmpl->prepare();
	...
	function return_tool_bar() {
		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'tool_bar.tpl');
		$tmpl->prepare();
	...
	function return_footer_box() {
		...
		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'footer.js');
		$tmpl->prepare();
		$tmpl->assign($this->label_array);
		// Output Javascript
		$out .= $tmpl->getOutputContent();

		$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower", $this->tmpl_path.'footer.tpl');
		$tmpl->prepare();
	...
	function single_cat () {
		...
		$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $message_text, 'link');
		...
		if (empty($this->cat)) $this->cat = t3lib_div::makeInstance("tx_chcforum_category", $this->cat_uid, $this->cObj);
	...
	function set_nav_path () {
		...
		$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $nav_path, $type);
	...
	function no_auth_msg ($type) {
		...
		$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $text, 'error');
		$out = $message->display();
		$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::makeLink(false, tx_chcforum_shared::lang('err_no_auth_msg_return')), 'link');
		$out .= $message->display();
		return $out;
	}
}

if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_dietaxforum/class.tx_ux_chcforum_display.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_dietaxforum/class.tx_ux_chcforum_display.php']);
}
