<?php
	...
	*/
	...
	class ux_tx_chcforum_user extends tx_chcforum_user {
...
*/
			...
			if ($GLOBALS["TSFE"]->loginUser && !$this->chcForumSesConf[$pid]['last_visit']) {
				...
				$where = 'feuser_uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
...
*/
		...
		function rate_post($rateSelect,$ratePostUID) {
			if ($this->fconf['allow_rating'] == true)  {
				...
				$fields = 'rating';
				if ($this->uid) {
					$addWhere = 'post_uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($ratePostUID,$table).' AND rater_uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);				
				} else {
					$addWhere = 'post_uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($ratePostUID,$table).' AND rater_ip='.$GLOBALS['TYPO3_DB']->fullQuoteStr($ip,$table);
				}
				$where = $addWhere;
		...
		*/
		function profile_form ($cwt_version = false) {
			...
			if ($cwt_version == true) { // one message for cwt, one for default
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_form_forum_hdr'), 'profile_hdr_big');
			} else {
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_form_hdr'), 'profile_hdr_big');
			}
			$out .= $message->display();
			...
			if ($this->fconf['mailer_disable'] != true) {
				...
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_form_email'), 'profile_hdr');
				$out .= $message->display();
			...
			if ($this->fconf['disable_img'] != true and $this->fconf['disable_img_edit'] != true) {
				if ($cwt_version == false) {
					$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_form_img'), 'profile_hdr');
					$out .= $message->display();
			...
			if ($this->fconf['disable_yahoo'] != true or $this->fconf['disable_aim'] != true or $this->fconf['disable_msn'] != true or $this->fconf['custom_im']) {
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_form_im'), 'profile_hdr');
				$out .= $message->display();
			...
			if ($this->fconf['disable_email'] != true) {			 
				if ($cwt_version == false) {
					$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_emailaddr'), 'profile_hdr');
					$out .= $message->display();
			...
			if ($this->fconf['disable_website'] != true) {
				if ($cwt_version == false) {
					$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_website'), 'profile_hdr');
					$out .= $message->display();
			...
			if ($cwt_version == false) {
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, tx_chcforum_shared::lang('profile_submit'), 'profile_hdr');
				$out .= $message->display();
			}
	...
	function mark_thread_read($thread_uid) {
		$thread = t3lib_div::makeInstance("tx_chcforum_thread", $thread_uid, $this->cObj);
		...
		*/
		function process_profile_form ($pv, $pf, $max_user_img = 102400) {			
			...
			if ($pv['submit'] && $pv['author'] == $this->uid) {
				...
				$message = t3lib_div::makeInstance("tx_chcforum_message", $this->cObj, $this->success_msg, 'link');
				$out .= $message->display();				
				RETURN $out;				
			}
		}
...
#$GLOBALS['TYPO3_DB']->store_lastBuiltQuery = TRUE;            
            $results = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            	'post_uid',
            	'tx_chcforum_posts_read',
            	'feuser_uid='.$this->uid
            );
#debug($GLOBALS['TYPO3_DB']->debug_lastBuiltQuery);            
		...
		function get_all_viewable_threads() {
			...
			if (is_array($confs)) {
				foreach ($confs as $v) {
					$a_conf = t3lib_div::makeInstance("tx_chcforum_conference", $v[uid], $this->cObj);
		...
		function return_new_in_conf($include_threads = false) {
			...
			if (isset($this->current_conf_id) && $this->uid) {
				$conf = t3lib_div::makeInstance("tx_chcforum_conference", $this->current_conf_id, $this->cObj);
		...
		*/
		function count_new_in_thread () {
			...
			if (isset($this->current_thread_id) && $this->uid) {
				$thread = t3lib_div::makeInstance("tx_chcforum_thread", $this->current_thread_id, $this->cObj);
		...
		*/
		...
		function set_access_array() {
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($results)) {
				...
				if (is_array($this->access_array['cats']) && array_key_exists($row['cat_id'],$this->access_array['cats']) == true) {
					...
					if ($row['auth_forumgroup_r'] == false && $this->uid == true) {
						$this->access_array['confs'][$row['uid']]['r'] = true;											
					}
					...
					// access to logged-in users only.
					if ($row['auth_forumgroup_w'] == false && $row['auth_forumgroup_r'] == false && $this->uid == true) {
						$this->access_array['confs'][$row['uid']]['w'] = true;											
					}
		...
		function can_edit_post($post_uid) {
			...
			if ($this->uid) { // a user must be logged in to edit a post...
				$post = t3lib_div::makeInstance("tx_chcforum_post", $post_uid, $this->cObj);
				...
				if ($post->post_author == $this->uid) {
					...
					$conference = t3lib_div::makeInstance("tx_chcforum_conference", $conf_uid, $this->cObj);
	...
	if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_dietaxforum/class.tx_ux_chcforum_user.php']) {
		include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vv_dietaxforum/class.tx_ux_chcforum_user.php']);
	}
