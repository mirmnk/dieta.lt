...
<?php
		...
		define('PATH_thisScript',str_replace('//','/', str_replace('\\','/', $HTTP_ENV_VARS["_"])));
...
#        require_once("$path_to_localconf");
        ...
        */
                ...
                foreach ($queued_not_sent as $message) {
                        ...
                        if ($message['pid'] != $current_pid) {
                                ...
                                if ($fconf['mailer_disable']['vDEF'] == 1) die;
																...
																$forum_pid = $fconf['forum_instance_pid'];
                        ...
                        $subject = '[dieta.lt forumas] '.$message[post_subject]. ' ('.$author_name.')';
		...
		if ($intMessSent) {
#			$strMess = $intMessSent . ' messages has been sent during this sesion';
#			sendMessage('m@vektorius.lt', '[Dieta.lt forum robot] report '.date('Y:m:d H:i:s'), $strMess);
			$query = "INSERT INTO tx_chcforum_mailsentlog(tstamp,messages) VALUES (".mktime()."," .$intMessSent.")";
            $query_results = mysql_query($query);
...
#        	require_once(PATH_t3lib."class.t3lib_div.php");
            ...
            if ($link_conf['view']) {
                    ...
                    $urlParameters['view'] = makeHash($link_conf['view']);
            } else {
                    $add_view = '';
            }
            ...
            if ($link_conf['thread_uid']) {
                    ...
                    $urlParameters['thread_uid'] = makeHash($link_conf['thread_uid']);                    
            }
...
# 			return $cObj->typoLink_URL($conf);           
        ...
        */
        function getAuthor($author_id) {
                $query = "SELECT username as name,email FROM fe_users WHERE uid=$author_id";
                $query_results = @mysql_query($query);
        ...
        */        
        function getRecipients($message) {
                ...
                while ($uid = mysql_fetch_assoc($results)) {
                        $query = "SELECT uid,username,usergroup,name,email FROM fe_users WHERE uid=$uid[user_uid] AND NOT deleted AND NOT disable";
                        $query_results = @mysql_query($query);
                        while ($user = @mysql_fetch_assoc($query_results)) {
